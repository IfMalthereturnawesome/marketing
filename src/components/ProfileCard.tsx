...
const ProfileInfo = ({
	...
	<div className='flex flex-col items-center gap-6 self-stretch'>
		<hgroup className='flex flex-col items-center justify-center gap-1 self-stretch'>
			...
			<p className='text-center text-sm font-normal text-secondary-foreground'>{handle}</p>
		</hgroup>
		<p className='line-clamp-3 text-center text-base font-normal text-secondary-foreground'>{description}</p>
	</div>
);
...
const ContactButton = () => (
	<a href='#'>
		<button
			type='button'
			className='w-full rounded bg-brand-primary px-4 py-2.5 text-center text-base font-medium text-white
		...
		<menu className='flex justify-center gap-4 self-stretch text-brand-primary'>
			{links.map(({ icon: Icon, label }) => (
				<li key={label}>
					...
					>
						<button
							className='flex justify-center gap-2 rounded p-2
								focus:shadow-[0px_0px_0px_1px_#444ce7,_0px_0px_0px_4px_rgba(68,_76,_231,_0.12)] disabled:bg-disabled
								disabled:text-disabled-foreground'
						>
							<Icon
...
const ProfileCard = ({ name, handle, description, image, className }: ProfileCardProps) => {
	return (
		<div
			className={cn(
				`shadow-marketing mx-auto flex w-[340px] flex-col items-center gap-10 rounded-lg border-none bg-primary px-4
				py-6`,
				className,
			)}
		>
